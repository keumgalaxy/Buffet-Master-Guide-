...
# 🧠 ตั้งค่าระบบความจำชั่วคราวส่วนกลาง ป้องกันตัวแปรหายเวลาเปลี่ยนหน้า
if "step" not in st.session_state:
    st.session_state.step = 1
if "g_items" not in st.session_state:
    ...
    * 🔲 **ถาดคอนโดชาบูทั่วไป** = 40-50 กรัม | 🍽️ **จานเปลไซส์กลาง** = 120 กรัม | 🥗 **จานเปลใหญ่** = 280 กรัม
    ...
    def menu_item_row(key_prefix, label_name):
        ...
        with c_left:
            ...
            sz = st.selectbox(f"ขนาด {label_name}:", ["เล็ก (~45g)", "กลาง (~120g)", "ใหญ่ (~280g)"], index=default_sz_idx, key=f"sz_{key_prefix}")
        with c_right:
    ...
    # 👑 ฝากค่าน้ำหนักของทุกอย่างลงตู้เซฟความจำส่วนกลางอย่างเป็นระบบ
    st.session_state.g_items = {
...
# --- STEP 3: หน้าสรุปผลลัพธ์สุดท้ายโดดๆ ---
    ...
    # ดึงค่าน้ำหนักผ่านตู้เซฟกลางทั้งหมด แก้บั๊ก NameError ได้เบ็ดเสร็จเด็ดขาด
